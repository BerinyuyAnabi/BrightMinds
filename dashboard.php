...
$user = $db->selectOne("
    SELECT u.role, u.username, c.childID 
    FROM users u 
...
if ($user && isset($user['role'])) {
    // If user is a child (has childID), allow access
    if ($user['childID']) {
        // User has a child record
    } elseif ($user['role'] === 'parent' && !$user['childID']) {
        ...
        <head>
            ...
            <style>
                ...
                .btn { display: inline-block; padding: 12px 24px; background: #667eea; color: white; text-decoration: none; border-radius: 10px; margin-top: 20px; transition: all 0.3s; }
        ...
        <body>
            <div class="card">
                ...
                <p>You are currently logged in as <strong>Parent</strong> (' . htmlspecialchars($user['username']) . ').</p>
                <p>This page is for Children.</p>
                <div style="display: flex; gap: 10px; justify-content: center; margin-top: 20px;">
                    <a href="parent-dashboard.php" class="btn">Go to Parent Dashboard</a>
...
<head>
    ...
    <title>Child Dashboard - Bright Minds</title>
    <link rel="stylesheet" href="css/main.css">
...
<body>
    ...
    <div class="dashboard">
        ...
        <div class="dashboard-header">
            <div class="user-profile">
                <div class="user-avatar" id="userAvatar"><?php 
                    ...
                    $avatarMap = ['owl' => '🦉', 'fox' => '🦊', 'rabbit' => '🐰', 'bear' => '🐻', 'cat' => '🐱', 'dog' => '🐶'];
                    $avatar = $childData['avatar'] ?? 'owl';
                    echo htmlspecialchars($avatarMap[$avatar] ?? '🦉'); 
                ?></div>
                <div>
        ...
        <!-- Parent Linking Section - Only shown when NOT linked -->
        <div class="parent-link-section" id="parentLinkSection" <?php echo isParentLinked($childId) ? 'class="hidden"' : ''; ?>>
            <!-- Not Linked View -->
            <div id="notLinkedView">
                ...
                <input 
                    ...
                    placeholder="Enter parent code (e.g., PAR-A7B3K)" 
                    maxlength="12">
        ...
        <div id="goalsSection" class="hidden" style="background: linear-gradient(135deg, #667eea 0%, #764ba2 100%); padding: 20px; border-radius: 15px; margin: 20px 0; color: white;">
            <h3 style="margin: 0 0 15px 0; display: flex; align-items: center; gap: 10px;">🎯 Your Goals</h3>
            <div id="goalsList">
                <div style="text-align: center; padding: 20px; opacity: 0.9;">Loading goals...</div>
            </div>
        </div>
